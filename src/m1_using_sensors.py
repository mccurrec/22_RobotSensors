...
class DriveSystem(object):
    ...
    def go_straight_for_inches_using_sensor(self, inches, speed):
        ...
        while True:
            current_position = abs(self.left_motor.get_position())
            if current_position >= desired_degrees:
                break
        self.stop()
    ...
    def go_straight_until_black(self, speed):
        ...
        self.go(speed, speed)
        while True:
            if color_sensor.get_reflected_light_intensity() <= 30:
                break
        self.stop()
